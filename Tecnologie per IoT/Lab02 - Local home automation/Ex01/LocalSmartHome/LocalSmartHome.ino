...
// presence detection using sound sensor variables
const int MIN_NOISE_EVENTS_IN_INTERVAL = 20;
const int MIC_TOL = 25;
...
unsigned long noiseTimeout;
boolean noisePeoplePresence;
...
/**
 * ISR for noise detection.
 * ISR executed each time a noise event is detected. It saves the time of each detection in a circuilar array
 * controlled using a counter that points to the cell that has to be written, so the cell with a the previous
 * counter is the last time recorded.
 * To remove dependencies with the sensor itself is used a time tollerance above which different recorded sounds
...
/**
 * Check for minimum suonds in an interval.
 * Controls if at least MIN_NOISE_EVENTS_IN_INTERVAL are recorded in the available time defined as noiseInterval.
 * If so it starts the period of time in which we can consider that someone is in the room even without
 * any other sound detection.
 */
...
/**
 * Check for peoplePresence in the room.
 * Controlling if any timeout has arrived to its end setting the related flag to false. If at least one
 * of the dedicated flags are still true sets the global variable that indicates people presence in the room.
 */
void presenceChecker()
{
    pirMovementDetection();  // PIR sensor detection with polling
    noiseCheckInterval();
    if (now - pirStartTime >= pirTimeout)
        pirPeoplePresence = false;
    if (now - noiseStartTime >= noiseTimeout)
        noisePeoplePresence = false;
    peoplePresence = (pirPeoplePresence || noisePeoplePresence) ? 1 : 0;  // beacuse peoplePresence is used as
                                                                          // index in bounds'temperature arrays
}
