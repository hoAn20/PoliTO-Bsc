...
class Catalog(object):
    ...
    def __init__(self):
        ...
        self.info = {
            "r": "http://192.168.1.12:8080/rest",
            "m": "http://192.168.1.12:8080/mqtt"
        }
        self.restInfo = {
            "d": "http://192.168.1.12:8080/devices/subscription/",
            "u": "http://192.168.1.12:8080/users/subscription/",
            "s": "http://192.168.1.12:8080/services/subscription/",
        }
        self.mqttInfo = {
        ...
        self.topic = self.mqttInfo['t'] + '+'
        self.messageBroker = self.mqttInfo['h']
    ...
    def GET(self, *path, **query):
        ...
        # Restituisce le liste complete dei dispositivi, servizi o utenti
            ...
            if path[0] == 'devices':
                ...
                return json.dumps(self.restInfo)
            elif path[0] == 'mqtt':
                return json.dumps(self.mqttInfo)
            else:
    ...
    def POST(self, *path, **query):
        ...
        if len(path) == 2:
            ...
            # Aggiunta di un nuovo servizio
                ...
                if path[1] == 'subscription':  # Per permettere l'aggiunta di funzionalità
                    # Controllo errori nel contenuto dei JSON ricevuti
                    if 'd' not in data or 'e' not in data:
    ...
    def PUT(self, *path, **query):
        ...
        if len(path) == 3:
            ...
            if path[0] == 'devices':
                ...
                return json.dumps({'t':'id', 'd': path[2]})
    ...
    def onMessageReceived(self, paho_mqtt, userdata, msg):
        ...
        reply_topic = self.mqttInfo['r'] + identification
        print(msg.topic, "received", msg.payload)
        ...
        try:
            ...
            # Controllo errori nel contenuto dei JSON ricevuti
            if 'e' not in data or 'r' not in data:
                publish.single(reply_topic, json.dumps({'t':'error', 'd':'Wrong device format, end-points and resources needed'}), hostname=self.messageBroker)
            elif identification not in self.devices:
                # Aggiunta di un nuovo dispositivo
                publish.single(reply_topic, json.dumps({'t':'id', 'd':self.addDevice(data)}), hostname=self.messageBroker)
            elif identification in self.devices:
                # Aggiornamento di un dispositivo
                self.updateDevice(data, identification)
                publish.single(reply_topic, json.dumps({'t':'id', 'd':identification}), hostname=self.messageBroker)
        except ValueError:
            # Controllo errori nel formato dei JSON ricevuti
            publish.single(reply_topic, json.dumps({'t':'error', 'd':'Wrong JSON format'}), hostname=self.messageBroker)


    # Metodi per aggiunta e aggiornamento di dispositivi, servizi e utenti
    def addDevice(self, data):
        udid = uuid.uuid4()
        did = str(udid).split('-')[0]
        self.devices[did] = {
            'id': did,
            'end-points': data['e'],
            'resources': data['r'],
        ...
        print('Added device: ', self.devices[did])
        return did
    ...
    def addService(self, data):
        ...
        print('Added service :', self.services[sid])
        return sid
