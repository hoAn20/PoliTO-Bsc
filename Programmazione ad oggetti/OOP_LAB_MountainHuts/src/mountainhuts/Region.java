...
public class Region {
	...
	private String name;
	private Map<Integer, Integer> altitudeRanges = new HashMap<>();
	...
	public void setAltitudeRanges(String... ranges) {
		for (String r: ranges) {
			String[] extremes = r.split("[-]");
			this.altitudeRanges.put(Integer.parseInt(extremes[0]), Integer.parseInt(extremes[1]));
		}
	}
	...
	public String getAltitudeRange(Integer altitude) {
		if (altitude == null)
			return "0-INF";
		for (int min: this.altitudeRanges.keySet()) {
			if (altitude >= min) {
				if (altitude <= this.altitudeRanges.get(min)) {
					return min + "-" + this.altitudeRanges.get(min);
				}
			}
		}
		return "0-INF";
	...
	public Municipality createOrGetMunicipality(String name, String province, Integer altitude) {
		if (this.municipalities.keySet().contains(name)) {
	...
	public MountainHut createOrGetMountainHut(String name, String category, Integer bedsNumber,
		...
		if (this.mountainHuts.keySet().contains(name)) {
	...
	public static Region fromFile(String name, String file) {
		...
		int cnt = 0;
		for (String l: Region.readData(file)) {
			if (cnt == 0) {
				cnt++;
			} else {
				String[] line = l. split("[;]+");
				Municipality mun = r.createOrGetMunicipality(line[1], line[0], Integer.parseInt(line[2]));
				if (line.length < 7) {
					r.createOrGetMountainHut(line[3], line[4], Integer.parseInt(line[5]), mun);
				} else {
					r.createOrGetMountainHut(line[3], Integer.parseInt(line[4]), line[5], 
							Integer.parseInt(line[6]), mun);
				}
			}
		}
		return r;
	...
	public Map<String, Map<String, Long>> countMountainHutsPerMunicipalityPerProvince() {
		return mountainHuts.values().stream()
				.collect(Collectors.groupingBy(
						(MountainHut m) -> m.getMunicipality().getProvince(),
						Collectors.groupingBy(
								(MountainHut m) -> m.getMunicipality().getName(),
								Collectors.counting()
								)
						)
				);
	...
	public Map<String, Long> countMountainHutsPerAltitudeRange() {
		return mountainHuts.values().stream()
				.map(m -> this.getAltitudeRange(m.getAltitude().orElse(m.getMunicipality().getAltitude())))
				.collect(Collectors.groupingBy(
						s -> s, 
						Collectors.counting()
						)
